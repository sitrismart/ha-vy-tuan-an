...
export default function App() {
  ...
  const handleShare = () => {
    if (navigator.share) {
      navigator.share({
        title: `Thiệp Cưới ${WEDDING_DATA.groom.shortName} & ${WEDDING_DATA.bride.shortName} | ${WEDDING_DATA.event.day}.${WEDDING_DATA.event.month}.${WEDDING_DATA.event.year}`,
        text: `Trân trọng kính mời bạn đến tham dự Lễ Thành Hôn của ${WEDDING_DATA.groom.shortName} & ${WEDDING_DATA.bride.shortName}!`,
  ...
  return (
    <main className="min-h-screen bg-[#2D1217] flex justify-center items-start text-[#2c1810] selection:bg-[#7A121D] selection:text-white font-body py-0 md:py-6 px-0 md:px-4">
      ...
      <div className="relative w-full max-w-[480px] min-h-screen bg-[#FAF6F0] shadow-2xl md:rounded-[40px] overflow-hidden z-10 border-0 md:border-8 md:border-[#3D1A21]/40 flex flex-col">
        ...
        <header className="sticky top-0 z-40 bg-[#FAF6F0]/90 backdrop-blur-md px-4 py-3 border-b border-[#7A121D]/10 flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5">
            ...
            <button
              onClick={handleShare}
              className="p-1.5 rounded-full hover:bg-[#7A121D]/10 text-[#7A121D] transition-colors text-xs flex items-center gap-1 font-medium"
              title="Chia sẻ thiệp cưới"
            >
              <Share2 className="w-4 h-4" />
              <span className="text-[10px] hidden sm:inline">
                {isCopiedLink ? 'Đã sao chép!' : 'Chia sẻ'}
              </span>
            </button>
            ...
            <button
              ...
              className="px-2.5 py-1 rounded-full bg-[#7A121D] text-white text-[11px] font-semibold tracking-wider flex items-center gap-1 hover:bg-[#600D16] transition-colors shadow-xs"
            >
        ...
        {/* 5. DRESSCODE SECTION: Color Palette */}
        <DresscodeSection />
        ...
        <nav aria-label="Điều hướng nhanh" className="sticky bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-[#7A121D]/15 px-3 py-2 flex items-center justify-around shadow-lg">
          ...
          >
            ...
            <span className="text-[9px] font-medium mt-0.5">Mừng Cưới</span>
          </button>
        </nav>
      ...
      {/* Gift Box / Banking QR Code Modal */}
      <GiftBoxModal
